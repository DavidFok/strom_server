...
module.exports = function makeDataHelpers(knex) {
  return {
    ...
    getCurrentChargeSessions(spotsArr) {
      // console.log("this is the array of spots inside getCurrentChargeSessions: ", spotsArr[0]);

      knex('sessions')
      .where({ spot_id: spotsArr[0].id })
      .orderBy('id', 'desc')
      .limit(1)
      .then((result) => {
        console.log("this is the result from the .max query: ", result);
      })

      // for (let spot of spotsArr) {
      //   if (spot.in_use) {
      //     spot[chargeEnd] = knex('sessions').max("id").where({ spot_id: spot.Id })
      //   } else {
      //     spot[chargeEnd] = null;
      //   }
      // }
